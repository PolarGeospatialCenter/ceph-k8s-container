...
function start_mds {
  ...
  if [ ! -e "$MDS_KEYRING" ]; then
    ...
    ceph "${CLI_OPTS[@]}" auth --keyring $MDS_BOOTSTRAP_KEYRING --name client.bootstrap-mds \
      get-or-create mds."$MDS_NAME" osd "allow rwx" mds "allow" mon "allow profile mds" -o "$MDS_KEYRING"
  ...
  exec /usr/bin/ceph-mds "${DAEMON_OPTS[@]}" --keyring $MDS_KEYRING -i "$MDS_NAME" -d
}
